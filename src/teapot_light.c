...
int main(int argc, char *argv[])
{
	...
	window = SDL_CreateWindow("FNA3D_HLSL Utah Teapot (SimpleDiffuse)",
	...
	effect_bytes = load_file("../assets/effects/diffuse.feb", &effect_len);
	...
	SDL_Log("Left-drag: orbit camera | Right-drag: orbit light | Scroll: zoom");
	...
	while (running)
	{
		...
		while (SDL_PollEvent(&evt))
		{
			...
			case SDL_EVENT_MOUSE_MOTION:
				...
				if (light_dragging)
				{
					light_azimuth -= evt.motion.xrel * 0.005f;
					light_altitude += evt.motion.yrel * 0.005f;
		...
		/* Light direction (points TOWARD light in world space) */
		{
			...
			FNA3D_SetEffectParamValue(device, effect, "LightDirection",
				ldir, 0, (uint32_t) sizeof(float) * 3);
		}

		{
			float diffuse[4] = {1.0f, 0.85f, 0.7f, 1.0f};
			FNA3D_SetEffectParamValue(device, effect, "DiffuseColor",
				diffuse, 0, (uint32_t) sizeof(float) * 4);
		}
